...
const Ql_user = () => {
  ...
  const fetchUsers = () => {
    fetch('https://anh-api-tong.onrender.com/admin/user/role')
      .then((response) => response.json())
      .then((data) => {
  ...
  const handleDelete = (id) => {
    if (window.confirm('Bạn có chắc muốn xóa người dùng này không?')) {
      fetch(`https://anh-api-tong.onrender.com/admin/user/${id}`, { method: 'DELETE' })
        .then(() => {
  ...
  const handleSave = () => {
   ...
   if (sp.sdt_user && !phoneRegex.test(sp.sdt_user)) {
    ...
    const url = `https://anh-api-tong.onrender.com/admin/user/${editUser || id}`;
    const opt = {
      method: 'PUT',
      headers: {
  ...
  const handleAddUser = async () => {
    ...
    try {
      ...
      const response = await fetch('https://anh-api-tong.onrender.com/admin/user', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
